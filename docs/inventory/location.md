...
| Location 조건 | 필수 계층값 |
|---|---|
| 공통 | `depthLv1`, 물리 단위 유형, Location 유형 |
| 피킹로케이션(`PICK`)·보관로케이션(`STRG`) 외 유형 | `depthLv3` 추가 필수 |
| 배송사 인계 Location(`SHIP`) | `depthLv4` 추가 필수 |

물리 구조상 계층 일부를 쓰지 않더라도 CGKR 생성 API가 요구하는 계층값은 입력해야 한다.

### 반품 Zone

한 센터에는 반품 Zone을 하나만 생성할 수 있다. 반품 프로세스를 시작하기 전에 센터에 반품 Zone이 존재해야 한다.

```text
반품 Zone 없음 → 반품 작업 시작 불가
반품 Zone 1개  → 사용 가능
반품 Zone 추가 생성 → 거부
```
